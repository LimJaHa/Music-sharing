...
def get_playlist_tracks_with_user(playlist_id):
    ...
    # 각 트랙의 이름, 아티스트, 추가한 사용자 ID, 트랙 순서 인덱스, 추가 시간을 반환
    return [(index, item['track']['name'], item['track']['artists'][0]['name'], item['added_by']['id'], item['added_at'])
            for index, item in enumerate(tracks)]

def get_tracks_by_users(tracks, user_ids):
    # 특정 사용자들이 추가한 트랙 목록을 반환
    user_tracks = [(index, track, artist, added_by, added_at) for index, track, artist, added_by, added_at in tracks if added_by in user_ids]
    return user_tracks

def print_tracks(tracks):
    for index, track, artist, added_by, added_at in tracks:
        print(f"Index: {index}, Track: {track}, Artist: {artist}, Added by: {added_by}, Added at: {added_at}")
